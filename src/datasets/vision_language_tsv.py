...
class VisionLanguageTSVDataset(object):
    ...
    def get_visual_data_refactoring(self, idx):
        """No start time, end time"""

        row = self.get_row_from_tsv(self.visual_tsv, idx)
        frames_dir = row[-1].split(".mp4")[0]
        frames = get_frames_from_dir(
            frames_dir,
            num_frames=self.decoder_num_frames,
        )
        return frames
    ...
    def __getitem__(self, idx):
        ...
        # get video frames
        # raw_frames: (T, C, H, W)
        raw_frames = self.get_visual_data_refactoring(img_idx)
        is_video = True
